...
def setup(group, rank, world_size):
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "29500"
    dist.init_process_group("gloo", rank=rank, world_size=world_size)

def dist_demo(group, rank, world_size, data_size_mb):
    setup(group, rank, world_size)
    ...
    for group in ['gloo', 'nccl']:
        for data_mb in [1, 10, 100, 1000]:
            for procs in [2, 4, 6]:
                ...
                execution_time = timeit.timeit(lambda: [mp.spawn(group=group, fn=dist_demo, args = (group, procs, data_mb, ), nprocs=procs, join=True), torch.cuda.synchronize()], number=1)
